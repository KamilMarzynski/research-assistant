...
describe("task persistence", () => {
  ...
  it("saveTask calls db.insert with correct values", async () => {
    ...
    const svc = makeTaskPersistence(db);
    const task = {
      taskId: "task-abc",
      projectId: "proj-1",
  ...
  it("migrateTasksFromJson does nothing when tasks dir missing", async () => {
    const db = mockDb();
    const svc = makeTaskPersistence(db);
    // tasks dir won't exist — should not throw
    await svc.migrateTasksFromJson();
...
describe("pending tools", () => {
  ...
  it("savePendingTool writes SKILL.md", async () => {
    ...
    await toolApproval.savePendingTool("fetch-arxiv", "# fetch-arxiv\n\nFetches papers.");
    const { readFile } = await import("node:fs/promises");
  ...
  it("savePendingTool writes script.py for Python script", async () => {
    ...
    await svc.ensureDirectories();
    const toolApproval = makeToolApproval();
    await toolApproval.savePendingTool(
      "run-analysis",
      "# run-analysis",
      "import pandas as pd\nprint('hi')",
    );
  ...
  it("savePendingTool writes script.sh for bash script", async () => {
    ...
    await svc.ensureDirectories();
    const toolApproval = makeToolApproval();
    await toolApproval.savePendingTool("run-bash", "# run-bash", "#!/bin/bash\necho hello");
    const { readFile } = await import("node:fs/promises");
    const content = await readFile(
  ...
  it("getPendingTools returns empty array when no pending tools", async () => {
    ...
    await svc.ensureDirectories();
    const toolApproval = makeToolApproval();
    expect(await toolApproval.getPendingTools()).toEqual([]);
  });
  ...
  it("getPendingTools returns saved tools", async () => {
    ...
    await svc.ensureDirectories();
    const toolApproval = makeToolApproval();
    await toolApproval.savePendingTool("tool-a", "# tool-a");
    await toolApproval.savePendingTool("tool-b", "# tool-b");
    const tools = await toolApproval.getPendingTools();
  ...
  it("approvePendingTool moves dir to skills/", async () => {
    ...
    await svc.ensureDirectories();
    const toolApproval = makeToolApproval();
    await toolApproval.savePendingTool("my-tool", "# my-tool");
    await toolApproval.approvePendingTool("my-tool");
    const { access } = await import("node:fs/promises");
    await expect(
  ...
  it("rejectPendingTool deletes the dir", async () => {
    ...
    await svc.ensureDirectories();
    const toolApproval = makeToolApproval();
    await toolApproval.savePendingTool("bad-tool", "# bad-tool");
    await toolApproval.rejectPendingTool("bad-tool");
    const { access } = await import("node:fs/promises");
...
describe("skill management", () => {
  ...
  it("getSkills returns array (builtin skills present after ensureDirectories)", async () => {
    ...
    const skills = await skillMgmt.getSkills();
  ...
  it("getSkills returns empty array when skills dir does not exist", async () => {
    ...
    expect(await skillMgmt.getSkills()).toEqual([]);
  });
  ...
  it("deleteSkill removes the skill directory", async () => {
    ...
    await skillMgmt.deleteSkill("deletable");
