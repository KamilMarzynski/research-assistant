...
## 2. `write_file` — Optimistic Locking with `fileHash`

### New Parameter

```ts
fileHash: Type.Optional(Type.String({
  description: "SHA-256 hash of the file content at the time of last read. If the file has changed since then, write will be rejected."
}))
```

### Behavior

1. If `fileHash` provided and file exists: compute current SHA-256 of file on disk.
2. If hashes don't match: throw `Error("File has changed since last read. Re-read the file and try again.")`.
3. If `fileHash` provided but file does not exist: proceed with write (new file).
4. Normal write flow continues: validate path, create dirs, write content.

### Agent Workflow

```
1. read_file(path: "/workspace/config.json")
   → { content: "...", fileHash: "abc123...", ... }

2. Agent edits content

3. write_file(path: "/workspace/config.json", content: "...", fileHash: "abc123...")
   → Success (file unchanged)
   → Or: Error "File has changed since last read" (concurrent edit detected)
```

---

## 3. `run_in_docker` Workspace File Mounting
...
| `write_file` fileHash mismatch | Throw `"File has changed since last read. Re-read and try again."` |
| `write_file` fileHash provided but file missing | Proceed with write (new file, no conflict) |
...
### `write_file` tests
- Write with matching `fileHash` → succeeds
- Write with mismatched `fileHash` → throws "File has changed"
- Write with `fileHash` but file does not exist → succeeds (new file)
- Write without `fileHash` → succeeds (no validation, backward compatible)
...
- Old `write_file` params: `{ path, content }` — still works
- New optional param: `{ path, content, fileHash }` — additive only
- `AgentToolName` stays `"read_file"` / `"write_file"` — no new tool names
...
- **Sandbox file replacement** — agent uses `write_file` with `fileHash` after `run_in_docker`. Optimistic locking prevents overwrites.
