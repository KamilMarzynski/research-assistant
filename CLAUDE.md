...
src/renderer/   — Browser context: React + Scholar design system + MUI v9, communicates only via window.electronAPI
...
- Stored in `Map<projectId, AgentSession>` in `src/main/ipc/session-manager.ts`
...
│   ├── memories/           — MemoryFileService .md files, one subdir per category
│   │   └── <category>/     — philosophy, decision, finding, tool_reference, project_convention
│   ├── skills/             — project-scoped skills
│   └── workspace/          — ephemeral task subdirs (one UUID per research run)
